...
logging.basicConfig(level=logging.INFO,
                    ...
                    )#datefmt='%Y-%m-%d %H:%M:%S'  # Custom format: YYYY-MM-DD HH:MM:SS)
...
class BadDateFixer:
    ...
    def fix_bad_date(self, recipe: Recipe):
        ...
        try:
            update_expression = "SET updated_at = :val" if recipe.updated_at else "REMOVE updated_at"
            expression_values = {":val": str(recipe.updated_at)} if recipe.updated_at else {}
            self.db.table.update_item(
                Key={"pk": recipe.pk, "sk": recipe.sk},
            ...
            logger.debug(f"No updated_at required for {recipe.recipe_id} ({recipe.title})")
            self.skipped_updated_at += 1

def main():
    parser = argparse.ArgumentParser(description="Fix bad created/updated dates in DynamoDB.")
    ...
    with open(
        ...
        items = [item for item in adapter.validate_json(f.read()) if isinstance(item, Recipe)]
    ...
    for idx, item in enumerate(items):
        try:
            ...
            if (idx + 1) % 50 == 0:
                logger.info(f"Updated {idx+1}/{len(items)} items. Skipped {fixer.skipped_updated_at} for updated_at.")
        except Exception as e:
            logger.error(f"Failed to fix item {idx+1}: {item}")
            raise e

    logger.info(f"Finished updating {len(items)} items in DynamoDB. Skipped {fixer.skipped_updated_at} for updated_at.")
