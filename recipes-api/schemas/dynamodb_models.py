...
class Recipe(BaseRecipes):
    ...
    def ser_datetime(self, value: datetime, handler: Any, info: FieldSerializationInfo):
        dynamo_db_mode = info.context.get(DYNAMODB_MODE, False) if info.context else False
...
def dynamo_db_item_discriminator(v):
    ...
    match sk.split("#")[0]:
        case "r" | "ar": return "Recipe"
        case _:
            raise Exception(f"Invalid sk: {sk}")

DynamoDbItem = Annotated[Union[Annotated[User, Tag("User")], Annotated[Recipe, Tag("Recipe")]], Discriminator(dynamo_db_item_discriminator)]