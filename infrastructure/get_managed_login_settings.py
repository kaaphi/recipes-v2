...
def get_stack_oauth_details(stack_name: str):
    ...
    return json.loads(next(output["OutputValue"] for output in outputs if output["OutputKey"] == "OAuthDetails"))
...
def main(config: Config) -> None:
    ...
    managed_login_settings = get_managed_login_settings(oauth_details["user_pool_id"], oauth_details["client_id"])
    with open(Path(__file__).parent.joinpath("infrastructure", "managed_login_settings.json"), 'w') as f:
